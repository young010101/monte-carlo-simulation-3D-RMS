...
__global__ void propagate(curandStatePhilox4_32_10_t *state, double *sig0,
                          double *dx2, double *dx4, double *sigRe,
                          double *NPar_count, const double *TD, const int TN,
                          const int NPar, const int Nbvec, const int Nc, const double res, const double *step,
                          const int NPix1, const int NPix2, const int NPix3,
                          const double *T2, const double *Pij,
                          const double *bval, const double *bvec, const int *APix)
{
    int idx = threadIdx.x + blockDim.x * blockIdx.x;
    ...
    for (int k = idx; k < NPar; k += stride)
    {
        ...
        while (1)
        {
            ...
            xParGi[0] = floor(xi[0]);
            xParGi[1] = floor(xi[1]);
            xParGi[2] = floor(xi[2]); // 0-99, 0-99, 0-179 & in IAS
            if (APix[NPix2 * NPix3 * xParGi[0] + NPix3 * xParGi[1] + xParGi[2]] != 0)
            {
                break;
            } // make sure par in intra-axon space (IAS)
        }
        ...
        for (int i = 0; i < TN; i++)
        {
            ...
            for (int j = 0; (j < nite) && (fstep > 0); j++)
            {
                ...
                // cy: Commented-out lines Above are about particle encounter 100*100*180 box edge. What is below?

                tmin = 2.0;
                ii_hit = -1;
                // cy: Compute the distance between par and box plane, tmp belong (-1, 1). No abs it in this step cuz blow.
                for (int ii = 0; ii < 3; ii++)
                {
                    if (vt[ii] > 0.0)
                    {
                        tmp = static_cast<double>(xParGi[ii]) + 1.0 - xt[ii]; // cy, >0
                    }
                    else if (vt[ii] < 0.0)
                    {
                        tmp = static_cast<double>(xParGi[ii]) - xt[ii]; // cy, <0
                    }
                    else
                    {
                        tmp = 2.0; // cy: > 1 just enough
                    }
                    ...
                    {
                        tt[ii] = max(0.0, tmp / vt[ii]); // cy, >0, why not converge with below?
                    }
                ...
                // cy: step means step_size = sqrt(6Dt)
                ...
                if (vt[ii_hit] >= 0.0)
                {
                    ...
                    // cy, if hit the plane of 100*100*180. Note: only change the vec. xt are change out of the loop.
                ...
                // cy, must encounter the box wall
                assert(!(ii_hit < 0));
                if (ii_hit < 0)
                {
                ...
                // cy, about permeatable. Base on the probility.
...
int main(int argc, char *argv[])
{
    ...
    thrust::host_vector<double> step(Nc); // Step size in IAS in �m
    for (i = 0; i < Nc; i++)
    {
    ...
    for (i = 0; i < timepoints * Nbvec; i++)
    {
        sigRe[i] = 0;
    }
    for (i = 0; i < timepoints * Nc; i++)
    {
        NPar_count[i] = 0;
    }
    ...
    std::ofstream fs0out("simulated_signal/sig0.txt");
    std::ofstream fdx2out("simulated_signal/dx2.txt");
    std::ofstream fdx4out("simulated_signal/dx4.txt");
    std::ofstream fsRout("simulated_signal/sigRe.txt");
    std::ofstream fNpout("simulated_signal/NPar_count.txt");
    fs0out.precision(15);
